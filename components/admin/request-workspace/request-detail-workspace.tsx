...
export function RequestDetailWorkspace({
  ...
  const handleAdvanceStage = () => {
    if (currentStageIndex < LIFECYCLE_STAGES.length - 1) {
      ...
      if (nextStage === "Approved" || nextStage === "Awaiting Payment") {
        setPendingStatus(nextStage);
  ...
  const handleStageSelect = (stage: RequestStatus) => {
    if (stage === "Approved" || stage === "Awaiting Payment") {
  ...
  const handleInvoiceSubmit = () => {
    if (pendingStatus) {
      updateRequestStatus(request.id, pendingStatus, invoiceNumberInput);
